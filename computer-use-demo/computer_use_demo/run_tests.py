...
def setup_state():
    ...
    if "test_case_executions" not in st.session_state:
        st.session_state.test_case_executions = []
...
async def main():
    ...
    for test_case in st.session_state.test_cases:
        ...
        container.write(f"**Test Case:** {test_case}")
        ...
        with st.spinner("Running Agent..."):
                ...
                st.session_state.messages = await sampling_loop(
                    ...
                    output_callback=partial(_render_message, chat=chat_container, sender=Sender.BOT),
                    tool_output_callback=partial(
                        _tool_output_callback, chat=chat_container, tool_state=st.session_state.tools
                    ),
                ...
                test_execution_log = _extract_test_execution_log(st.session_state.messages)
                test_case_execution["test_result"] = "Pass" if "TEST_RESULT: PASSED" in str(test_execution_log[0]) else "Fail"
                test_case_execution["test_execution_log"] = test_execution_log[0]
                st.session_state.messages = []
                container.write(f"**Result:** {test_case_execution['test_result']}")   
                container.write("**Test Execution Log:**")            
                container.json(test_execution_log[0])

        test_number += 1

        st.session_state.test_case_executions.append(test_case_execution)

    _render_download_button(json.dumps(st.session_state.test_case_executions))
...
def _tool_output_callback(
    tool_output: ToolResult, tool_id: str, tool_state: dict[str, ToolResult], chat: DeltaGenerator
):
    """Handle a tool output by storing it to state and rendering it."""
    tool_state[tool_id] = tool_output
    _render_message(tool_output, Sender.TOOL, chat)
